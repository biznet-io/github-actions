...
describe('SSHHelper', () => {
  ...
  it('initializes SSH configuration', async () => {
    ...
    expect(exec.exec).toHaveBeenCalledWith('ssh-agent', ['-a', mockSocketPath])

    // Verify known hosts were configured
    expect(exec.exec).toHaveBeenCalledWith('ssh-keyscan', ['-H', 'github.com'], expect.any(Object))

    // Verify SSH key was added
    expect(exec.exec).toHaveBeenCalledWith('ssh-add', ['-'], expect.any(Object))
  })

  it('handles missing SSH key', async () => {
    delete process.env['SSH_PRIVATE_KEY']

    await expect(sshHelper.initialize()).rejects.toThrow('SSH_PRIVATE_KEY secret is not set')
  })
  ...
  it('performs cleanup', async () => {
    ...
    expect(exec.exec).toHaveBeenCalledWith('ssh-add', ['-D'], expect.any(Object))
  })
